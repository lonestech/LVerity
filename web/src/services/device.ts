...
export const deviceService = {
   ...
   * @returns 注销结果
   */
  unregisterDevice: (id: string): Promise<ApiResponse<void>> => {
    return request.post(`/api/devices/${id}/unregister`);
  },
   ...
   * @returns 重置结果
   */
  resetDeviceFilters: (): Promise<ApiResponse<void>> => {
    return request.post('/api/devices/reset-filters');
  },

  /**
   * 批量管理设备
   * @param ids 设备ID列表
   * @param action 操作类型 (activate/deactivate/delete/unregister)
   * @returns 操作结果
   */
  batchManageDevices: (ids: string[], action: 'activate' | 'deactivate' | 'delete' | 'unregister'): Promise<ApiResponse<void>> => {
    return request.post('/api/devices/batch', { ids, action });
  }
};
