...
export const systemService = {
  ...
  // 获取单个系统配置
  getSystemConfigByName: async (name: string): Promise<ApiResponse<SystemConfig>> => {
    return request.get(`/api/system/config/${name}`);
  },
  ...
  // 更新系统配置
  updateSystemConfig: async (name: string, value: string): Promise<ApiResponse<SystemConfig>> => {
    return request.put(`/api/system/config/${name}`, { value });
  },
  
  // 删除系统配置
  deleteSystemConfig: async (name: string): Promise<ApiResponse<void>> => {
    return request.delete(`/api/system/config/${name}`);
  },
  
  // 初始化系统配置
  initSystemConfigs: async (): Promise<ApiResponse<void>> => {
    return request.post('/api/system/config/init');
  },
  
  // 为了兼容旧代码，提供重置配置功能的别名
  resetSystemConfig: async (): Promise<ApiResponse<void>> => {
    return systemService.initSystemConfigs();
  }
};
