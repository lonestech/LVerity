...
export const licenseService = {
  ...
  createLicense: (license: Partial<License>): Promise<ApiResponse<License>> => {
    ...
    const apiLicense = {
      ...
      group_id: license.customerId || '',
      expires_at: license.expiryDate || license.expires_at,
      usage_limit: 0,
      tags: []
    };
    
    console.log('转换后的API请求数据:', apiLicense);
    return request.post('/api/licenses', apiLicense);
  },
  ...
  batchGenerateLicenses: (params: {
    ...
    return request.post('/api/licenses/batch-generate', params);
  },
  ...
  generateLicenseKey: (): Promise<ApiResponse<{ key: string }>> => {
    return request.get('/api/licenses/generate-key');
  },
