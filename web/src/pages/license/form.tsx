  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      if (values.dateRange && values.dateRange.length === 2) {
        ...
        // 后端API需要的格式转换
        licenseData.type = licenseData.type || 'standard'; // 确保有默认类型
        licenseData.max_devices = licenseData.maxDevices; // 添加下划线格式的字段
        licenseData.expires_at = licenseData.expiryDate; // 添加下划线格式的字段
        licenseData.group_id = licenseData.groupId || ''; // 添加下划线格式的字段
      }
